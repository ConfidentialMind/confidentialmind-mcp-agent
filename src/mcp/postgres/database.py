...
class DatabaseManager:
    ...
    async def get_connection_pool(self) -> Pool:
        ...
        if self.log_interval_count == 0 and self.pool:
            ...
            used_percent = (
                (free_connections / self.pool.get_size()) * 100 if self.pool.get_size() > 0 else 0
            )
            logger.info(
    ...
    async def execute_query(self, query: str) -> Tuple[bool, Union[List[Dict[str, Any]], str]]:
        ...
        try:
            ...
            logger.error(
                f"Query timeout after {self.settings.statement_timeout}ms: {query_preview}"
            )
            return (
                False,
                f"Query timed out after {self.settings.statement_timeout / 1000:.1f} seconds",
            )
    ...
    ) -> Dict[str, Union[str, List[Dict[str, Any]], List[str]]]:
