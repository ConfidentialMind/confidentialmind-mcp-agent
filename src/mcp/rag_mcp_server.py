...
class RAGHandler:
    ...
    def handle_list_tools(self) -> ListToolsResponse:
        ...
        return ListToolsResponse(
            tools=[
                ...
                ToolDefinition(
                    name="rag_generate_completion",
                    description="Generates a response from the RAG system, augmented with retrieved context. Uses OpenAI-compatible inputs.",
                    inputSchema=ToolInputSchema(
                        type="object",
                        properties={
                            "messages": {
                                "type": "array",
                                "description": "The messages to generate a completion for. Can be a single string (will be converted to a user message) or an array of {role, content} objects.",
                            },
                            "model": {
                                "type": "string",
                                "description": "The model to use for generation. Default: 'cm-llm'",
                            },
                            "temperature": {
                                "type": "number",
                                "description": "The temperature to use for generation. Default: 0.7",
                            },
                            "max_tokens": {
                                "type": "integer",
                            ...
                            "max_chunks": {
                                "type": "integer",
                                "description": "Maximum number of context chunks to use. Default: 3",
                            },
                            "stream": {
                                "type": "boolean",
    ...
    def _handle_generate_completion(self, arguments: Dict[str, Any]) -> CallToolResponse:
        """Handle completion generation from RAG system"""
        if "messages" not in arguments:
            raise ValueError("Missing 'messages' argument for 'rag_generate_completion' tool")

        try:
            # Format messages if they're not already in the correct format
            messages = arguments.get("messages", [])
            formatted_messages = []

            # Check if messages need formatting (if they're just strings or not in expected format)
            ...
            for msg in messages:
                if isinstance(msg, str):
                    ...
                    # Message is a dict but missing required fields
            ...
            # Replace the original messages with properly formatted ones
            arguments["messages"] = formatted_messages

            # Ensure other required parameters are present
            if "model" not in arguments:
                arguments["model"] = "cm-llm"  # Default model

            logging.info(
                f"Requesting completion with {len(arguments.get('messages', []))} messages"
            )
            ...
            logging.debug(f"RAG completion request payload: {json.dumps(arguments)}")

            # Make the API request
            # Ensure we're using the correct endpoint - if the base URL already includes /v1, don't duplicate it
            chat_completions_url = f"{self.rag_base_url}/v1/chat/completions"

            logging.info(f"Making completion request to: {chat_completions_url}")
            ...
            if stream:
                # For streaming, accumulate the full response
                full_response = self._accumulate_streaming_response(response)
                result_json = json.dumps(full_response, indent=2)
            else:
                # For non-streaming, just get the JSON response
                result = response.json()
                result_json = json.dumps(result, indent=2)

            logging.info("Completion generation successful")
            return CallToolResponse(content=[TextContent(text=result_json)], isError=False)
            ...
            logging.error(f"Completion generation HTTP error: {status_code} - {e}", exc_info=True)
            return CallToolResponse(
                content=[
                    TextContent(
                        text=f"RAG Completion Error ({status_code}): {str(e)}. {error_detail}"
                    )
                ],
                isError=True,
            )
        except requests.exceptions.ConnectionError as e:
            logging.error(f"Completion generation connection error: {e}", exc_info=True)
            return CallToolResponse(
                content=[
                    TextContent(
                        text=f"RAG Connection Error: Could not connect to the RAG service at {self.rag_base_url}. Please check if the service is running and the URL is correct."
            ...
            logging.error(f"Completion generation error: {e}", exc_info=True)
            return CallToolResponse(
                content=[TextContent(text=f"RAG Completion Error: {str(e)}")],
                isError=True,
            )
